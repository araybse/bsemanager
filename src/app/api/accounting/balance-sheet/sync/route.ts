...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `https://quickbooks.api.intuit.com/v3/company/${settings.realm_id}/reports/BalanceSheet?${query.toString()}`
    ...
    if (!response.ok) {
      ...
      throw new Error(`QBO BalanceSheet API error ${response.status}: ${errorText}`)
    }

    const reportPayload = await response.json()
    const headerEndPeriod = String(reportPayload?.Header?.EndPeriod || '').slice(0, 10)
    if (headerEndPeriod && headerEndPeriod !== periodEnd) {
      throw new Error(
        `QBO BalanceSheet returned EndPeriod ${headerEndPeriod} instead of requested ${periodEnd}. Snapshot was not saved.`
      )
    }

    const rows = ((reportPayload?.Rows?.Row as QboReportRow[] | undefined) || [])
    ...
    if (snapshotError || !snapshot) {
      throw snapshotError || new Error('Failed to persist balance sheet snapshot')
    }
    ...
    return NextResponse.json(
      { error: (error as Error).message || 'Failed to sync Balance Sheet snapshot' },
      { status: 500 }
    )
  }
}

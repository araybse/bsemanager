...
export default function CashFlowPage() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // Calculate monthly change in distributions (negative balance change = distribution paid out)
      const sortedMonths = Array.from(distributionsBalanceByMonth.keys()).sort()
      sortedMonths.forEach((month, idx) => {
        if (idx === 0) return // Skip first month (no prior to compare)
        const currentBalance = distributionsBalanceByMonth.get(month) || 0
        const priorMonth = sortedMonths[idx - 1]
        const priorBalance = distributionsBalanceByMonth.get(priorMonth) || 0
        const change = currentBalance - priorBalance
        // Distributions account is negative equity, so a more negative value = distribution paid
        // Change will be negative when distributions are paid out
        if (change !== 0) {
      ...
      const { data: distributionsRows, error: distributionsError } = await supabase
        .from('cash_flow_distributions' as never)
        .select('month, amount')
        .gte('month' as never, `${COMPANY_START_YEAR}-05` as never)
      ...
      const distributionsByMonth = new Map<string, number>()
      
      // Start with historical data from QuickBooks Balance Sheet
      historicalDistributionsByMonth.forEach((amount, month) => {
        distributionsByMonth.set(month, amount)
      })
      
      // Override/add manual forecasts for current and future months
      ;((distributionsRows as { month: string; amount: number }[] | null) || []).forEach((row) => {
        const month = (row.month || '').slice(0, 7)
        if (!month) return
        // Manual data takes precedence for current/future months
        if (month >= currentMonthKey) {
          distributionsByMonth.set(month, Number(row.amount) || 0)
        }
      })
      ...
      months.forEach((month) => {
        ...
        const distributions = distributionsByMonth.get(month) || 0
        ...
        const ending = isCompletedMonth && hasActualEnding ? actualEnding! : starting + netIncome - distributions
                ...
                <Table>
                  ...
                  <TableBody>
                    <TableRow className="bg-muted/10">
                      <TableCell className="sticky left-0 bg-background font-semibold border-r z-10">
                      ...
                      : visibleIncomeCategoryNames.map((accountName) => (
                          <Fragment key={`income-group::${accountName}`}>
                            <TableRow key={`income-child::${accountName}`}>
                              <TableCell className="sticky left-0 bg-background pl-8 border-r z-10">
                                ...
                                : visibleServiceProjectGroups.map((group) => {
                                    ...
                                    return (
                                      <Fragment key={`services-year::${group.year}`}>
                                        <TableRow>
                                          <TableCell className="sticky left-0 bg-background pl-10 font-medium border-r z-10">
                                            <button
                                              type="button"
                                              className="inline-flex items-center gap-1"
                                              onClick={() =>
                                        ...
                                        {yearCollapsed
                                          ...
                                          : group.projects.map((project) => {
                                              ...
                                              return (
                                                <Fragment key={`services-group::${projectKey}`}>
                                                  <TableRow key={`services-project::${projectKey}`}>
                                                    <TableCell className="sticky left-0 bg-background pl-12 border-r z-10">
                                            ...
                                            <TableRow key={`services-phase::${projectKey}::${phase.phaseKey}`}>
                                              <TableCell className="sticky left-0 bg-background pl-16 text-muted-foreground border-r z-10">
                                                {phase.phaseName}
                                              </TableCell>
                            ...
                            {visibleExpenseSubsections.map((subsection) => {
                              ...
                              return (
                                <Fragment key={subsection.key}>
                                  <TableRow>
                                    <TableCell className="sticky left-0 bg-background pl-8 font-medium border-r z-10">
                                      <button
                                        type="button"
                                        className="inline-flex items-center gap-1"
                                        onClick={() =>
                    ...
                    <TableRow className="bg-muted/20">
                      <TableCell className="sticky left-0 bg-background font-semibold border-r z-10">Net Income</TableCell>
                      {metricEmptyCells}
                      {tableColumns.map((column) => {
                    ...
                    <TableRow>
                      ...
                      {tableColumns.map((column) => {
                        ...
                        const isHistorical = month < (cashFlowData?.currentMonthKey || '')
                        const currentValue = cashFlowData?.distributionsByMonth?.get(month) || 0
                        const cellId = `distribution::${month}`
                        const manualValue = manualEdits[cellId]
                        const displayValue = manualValue !== undefined ? (manualValue === '' ? 0 : parseFloat(manualValue)) : currentValue

                        return (
                          <TableCell
                            key={cellId}
                            className={`text-right font-mono ${currentBoundaryClass(month)}`}
                          >
                            {isHistorical ? (
                              <span className={displayValue < 0 ? 'text-red-600' : ''}>{formatCurrency(displayValue)}</span>
                            ) : (
                              <Input
                                type="number"
                                step="0.01"
                                value={displayValue}
                                onChange={(e) => {
                                  setManualEdits((prev) => ({ ...prev, [cellId]: e.target.value }))
                                }}
                                onBlur={async () => {
                                  const val = manualEdits[cellId]
                                  ...
                                  try {
                                    const { error } = await supabase
                                      .from('cash_flow_distributions' as never)
                                      .upsert(
                                        { month, amount: numVal } as never,
                                        { onConflict: 'month' as never }
                                      )
                                    if (error) throw error
                                    queryClient.invalidateQueries({ queryKey: ['cash-flow-table'] })
